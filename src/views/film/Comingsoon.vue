...
export default {
  ...
  mounted () {
    http(`/api/gateway?cityId=${this.$store.state.cityId}&pageNum=1&pageSize=10&type=2`, {
      headers: {
  ...
  methods: {
    ...
    onLoad () {
      ...
      this.current++
      http(`/api/gateway?cityId=${this.$store.state.cityId}&pageNum=${this.current}&pageSize=10&type=2`, {
        headers: {
          'X-Host': 'mall.film-ticket.film.list'
        }
      }).then(res => {
